...
#region Echo
        private async Task Echo(HttpContext context, WebSocket webSocket)
        {
            ...
            var incoming = await webSocket.ReceiveAsync(new ArraySegment<byte>(buffer), CancellationToken.None);
            for (;;)
            {
                var outbuffer = Encoding.ASCII.GetBytes(DateTime.Now.ToString(CultureInfo.InvariantCulture));
